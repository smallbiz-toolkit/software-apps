...
function createCard(item) {
  let imgUrl = item.largeImageURL;
  let imgThumbnailUrl = item.webformatURL;
  let card = `
      <div class="col" style="margin-bottom:25px;">
          <div class="card h-100">
              <img src="${imgThumbnailUrl}" class="card-img-top img-preview" alt="${item.tags}" data-bs-toggle="modal" data-bs-target="#previewModal" data-img-url="${imgThumbnailUrl}">
...
// Event to open modal and show image when click on thumbnail
...
function loadPage(page) {
   ...
   $.getJSON('test-json.php?' + queryString, function(data) {
      ...
      // Check if data.hits is empty
      if (data.hits.length === 0) {
        gallery.html(`
          <div class="col-12 margin-top-25">
            <h5 class="text-center">Sorry, there are no more images (or matching images), please revise search.</h5>
            <div class="text-center">
              <img src="https://qiksoft.com/qiksoft_logo_200.png" alt="QikSoft">
            </div>
          </div>
        `);
      } else {
